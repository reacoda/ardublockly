...
Blockly.Blocks['stepper_config'] = {
  ...
  init: function() {
    ...
    this.appendDummyInput()
        .appendField('Set dial variable to')
        .appendField(
            new Blockly.FieldInstance('Foward',
                                      'Backward',
                                      true, true, false),
            'STEPPER_NAME');
    ...
    this.appendDummyInput('PINS')
       // .setAlign(Blockly.ALIGN_RIGHT)
        .appendField('Set dial to port')
        .appendField(new Blockly.FieldDropdown(
            Blockly.Arduino.Boards.selected.ports), 'STEPPER_PIN1');
        //.appendField(Blockly.Msg.ARD_STEPPER_PIN2)
        //.appendField(new Blockly.FieldDropdown(
          //  Blockly.Arduino.Boards.selected.ports), 'STEPPER_PIN2');
    ...
    if (fourPins) {
      if (!extraPinsExist) {
         this.getInput("PINS")
            ...
            .appendField(new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.ports), 'STEPPER_PIN3')
            .appendField(Blockly.Msg.ARD_STEPPER_PIN4, "PIN4")
            .appendField(new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.ports), 'STEPPER_PIN4');
      }
    } else {
  ...
  updateFields: function() {
    Blockly.Boards.refreshBlockFieldDropdown(
        this, 'STEPPER_PIN1', 'ports');
    Blockly.Boards.refreshBlockFieldDropdown(
        this, 'STEPPER_PIN2', 'ports');
    Blockly.Boards.refreshBlockFieldDropdown(
...
Blockly.Blocks['stepper_step'] = {
  ...
  init: function() {
    ...
    this.setColour(Blockly.Blocks.stepper.HUE);
    this.appendDummyInput()
        .appendField("Drive Motor at Port F")
        .appendField(
            new Blockly.FieldInstance('Backward',
                                      'Forward',
                                      false, true, false),
            'STEPPER_NAME');
    this.appendDummyInput()
